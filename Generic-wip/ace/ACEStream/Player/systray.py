...
class PlayerOptionsDialog(wx.Dialog):
	...
	import os
        ...
        string = f.read()
        player_buffer_time = self.bgapp.get_playerconfig('player_buffer_time', int(string))
	    ...
	    import os
            ...
            string = f.read()
            live_buffer_time = self.bgapp.get_playerconfig('live_buffer_time', int(string))
    ...
    def OnApply(self, event = None):
	    ...
	    import os
            playerbuffervalue_file = os.path.join(os.path.split(current_file_path)[0],"values","vodbuffer.txt")
            f = open(playerbuffervalue_file, "r")
            string = f.read()
            player_buffer_time = int(string)
